...
# ============================================================================
# Why: We need realistic data that demonstrates bias patterns
# Business context: Credit risk assessment for loan approvals
...
# Generate target variable (risk: 1 = good credit, 0 = bad credit)
# Business logic: Higher risk for:
# - Lower credit scores
# - Higher debt-to-income ratio
# - More late payments
risk_score = (
    -df['credit_score'] / 1000 +           # Lower score = higher risk
    df['debt_to_income'] * 3 +              # High debt = higher risk
    df['late_payments'] * 0.5 +             # Late payments = higher risk
...
# ============================================================================
# Why: Models need numerical, scaled features. We'll train a Random Forest
# Business context: Random Forest provides feature importance and handles non-linear relationships
...
# ============================================================================
# SECTION 4: MODEL SERIALIZATION (SAVING FOR DEPLOYMENT)
# ============================================================================
# Why: Models must be saved to disk to be loaded by production applications
# Business context: The trained model is an asset that needs versioning and storage

print("\n" + "=" * 80)
print("STEP 3: MODEL SERIALIZATION - Saving for Deployment")
print("=" * 80)

# Method 1: Pickle (Python's native serialization)
print("\nSaving with pickle...")
with open('credit_model.pkl', 'wb') as f:
    pickle.dump(model, f)
print("   ✅ Saved as 'credit_model.pkl'")

# Method 2: Joblib (optimized for scikit-learn models)
print("\nSaving with joblib...")
joblib.dump(model, 'credit_model.joblib')
print("   ✅ Saved as 'credit_model.joblib'")

# Save preprocessing objects (CRITICAL for consistency!)
print("\nSaving preprocessing objects...")
joblib.dump(scaler, 'scaler.joblib')
joblib.dump(encoders, 'label_encoders.joblib')
print("   ✅ Saved scaler.joblib and label_encoders.joblib")
...
if pred_original == pred_loaded:
    print("   ✅ Model loads correctly! Ready for deployment.")
else:
...
# ============================================================================
# Why: Production models need APIs to serve predictions to applications
# Business context: REST API enables integration with web/mobile apps
...
def predict():
    ...
    Expected JSON with all features
    """
    try:
...
# Write API file to disk
with open('app.py', 'w') as f:
    f.write(api_code)

print("\n✅ Flask API template saved as 'app.py'")
...
print("        -d '{\"age\":45,\"gender\":\"Male\",\"income\":60000,\"loan_amount\":20000,\"credit_score\":720,\"debt_to_income\":0.3,\"employment_years\":10,\"late_payments\":0,\"education\":\"Bachelor\",\"marital_status\":\"Married\"}'")


# ============================================================================
# SECTION 6: FAIRNESS AUDIT - BIAS DETECTION
# ============================================================================
# Why: Models can discriminate against protected groups
# Business context: Regulatory compliance (GDPR, ECOA) and ethical responsibility
...
# Add age groups for demographic analysis
df['age_group'] = pd.cut(
    df['age'], 
    bins=[0, 30, 50, 100], 
...
# DIR < 0.8 indicates potential discrimination (80% rule)
...
print(f"   {'✅ PASS' if disparate_impact_gender >= 0.8 else '⚠️ FLAG - Possible discrimination'}")
...
print(f"   {'✅ PASS' if disparate_impact_age >= 0.8 else '⚠️ FLAG - Possible age discrimination'}")

# ========== ERROR RATE ANALYSIS ==========
print("\n📊 3. ERROR RATE ANALYSIS (False Positives & Negatives)")
print("-" * 50)
print("False Positive: Bad loan approved (model said Good, actually Bad)")
print("False Negative: Good loan rejected (model said Bad, actually Good)")
...
for gender in ['Male', 'Female']:
    ...
    print(f"\n{gender}:")
    print(f"   False Positive Rate (bad loans approved): {fp_rate:.3f} ({fp}/{total_bad})")
    print(f"   False Negative Rate (good loans rejected): {fn_rate:.3f} ({fn}/{total_good})")
...
if bias_flags:
    print("\n⚠️ BIAS ALERT: The following issues were detected:")
    for flag in bias_flags:
        print(f"   • {flag}")
    print("\n🔧 RECOMMENDED ACTIONS:")
    print("   1. Collect more diverse training data")
    print("   2. Consider fairness-aware algorithms (e.g., Fairlearn)")
    print("   3. Implement human oversight for borderline cases")
    print("   4. Add fairness constraints to model optimization")
    print("   5. Conduct regular fairness audits")
else:
    print("\n✅ FAIRNESS CHECK PASSED: No severe disparate impact detected")
    print("   Continue monitoring quarterly for drift")


# ============================================================================
# SECTION 7: MODEL CARD GENERATION
# ============================================================================
# Why: Model cards document limitations and intended use
# Business context: Transparency for regulators, customers, and internal teams
...
model_card = {
    ...
    "Training Data": {
        "Source": "Synthetic dataset (simulated customer data)",
    ...
    "Performance Metrics": {
        ...
        "Precision": f"{classification_report(y_test, model.predict(X_test), output_dict=True)['1']['precision']:.3f}",
        "Recall": f"{classification_report(y_test, model.predict(X_test), output_dict=True)['1']['recall']:.3f}",
        "F1-Score": f"{classification_report(y_test, model.predict(X_test), output_dict=True)['1']['f1-score']:.3f}"
    },
    "Fairness Assessment": {
    ...
    "Monitoring Plan": {
        ...
        "Alert Thresholds": {
            "Disparate Impact": "< 0.8 for 2 consecutive quarters",
            "Accuracy Drop": "> 5% decline",
            "Data Drift": "PS > 0.1"
        }
    }
}
...
# ============================================================================
# Why: Models need to demonstrate business value
# Business context: ROI calculation for stakeholders
...
cost_per_call = 2.50      # Cost to contact a customer
deposit_value = 50.00     # Profit from successful deposit
...
# Strategy 2: Target top 30% by model probability
...
# Strategy 3: Target top 50% by model probability
threshold_50 = np.percentile(y_proba, 50)
targeted_customers_50 = y_proba >= threshold_50
calls_targeted_50 = targeted_customers_50.sum()
cost_targeted_50 = calls_targeted_50 * cost_per_call
successes_targeted_50 = y_test[targeted_customers_50].sum()
profit_targeted_50 = successes_targeted_50 * deposit_value - cost_targeted_50

print(f"\n🎯 Strategy 3: Model-targeted (Top 50%)")
print(f"   Calls: {calls_targeted_50}")
print(f"   Cost: ${cost_targeted_50:,.2f}")
print(f"   Expected successes: {successes_targeted_50}")
print(f"   Profit: ${profit_targeted_50:,.2f}")

# Calculate ROI and improvement
print("\n📈 BUSINESS IMPACT SUMMARY")
print("-" * 60)

best_strategy = "Top 30% Targeting"
best_profit = profit_targeted
improvement = profit_targeted - profit_all
roi_improvement = ((profit_targeted / cost_targeted) / (profit_all / cost_all) - 1) * 100

print(f"\n✅ Best Strategy: {best_strategy}")
print(f"   Profit improvement over baseline: ${improvement:,.2f}")
...
print("\n💡 KEY INSIGHT:")
print("   The model doesn't just predict better - it saves money!")
print("   By targeting only the most promising customers, we achieve")
print("   higher profit with fewer resources.")
...
# ============================================================================
# Why: Models degrade over time; need ongoing monitoring
# Business context: Proactive risk management
...
2. DATA COLLECTION
   ...
   [✓] Track actual outcomes (loan performance)
...
# Save monitoring plan to file with UTF-8 encoding
with open('monitoring_plan.txt', 'w', encoding='utf-8') as f:
    f.write(monitoring_plan)
print("\n✅ Monitoring plan saved as 'monitoring_plan.txt'")


# ============================================================================
# SECTION 10: FINAL SUMMARY AND DELIVERABLES
# ============================================================================
...
1. DEPLOYMENT CREATES VALUE: A model in a notebook has zero value.
   Real value comes from integrating models into business processes.

2. MODELS CAN PERPETUATE HARM: Historical bias in data leads to
   discriminatory predictions. Always audit for fairness.

3. FAIRNESS IS MEASURABLE: Use metrics like Disparate Impact Ratio,
   false positive rates, and subgroup analysis to detect bias.

4. TRANSPARENCY BUILDS TRUST: Document limitations in model cards
   and be able to explain decisions to stakeholders.

5. MONITORING IS CONTINUOUS: Models degrade over time. Implement
   ongoing monitoring for performance and fairness drift.

6. ETHICS IS GOOD BUSINESS: Fair, transparent models build customer
   trust and protect against regulatory and reputational risk.
""")

print("\n" + "=" * 80)
print(f"✅ EXERCISE COMPLETED SUCCESSFULLY at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("=" * 80)